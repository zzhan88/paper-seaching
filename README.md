...
每天自动从多个学术数据源检索 AI、酶工程、蛋白质工程和生物催化方向的新论文，经主题准入、期刊适配、去重和多样性排序后，通过邮件推送轻阅读日报。

## 推荐逻辑

系统不再把“出现 enzyme 或 protein”直接视为相关。候选论文需要依次通过：

1. **多源召回**：OpenAlex、PubMed、Crossref 并行检索；单个来源失败不会中断其他来源。
2. **统一去重**：优先使用规范化 DOI；无 DOI 时使用规范化标题。
3. **主题硬门槛**：必须同时包含酶/生物催化研究对象，以及设计、改造、筛选或性能评价行为。
4. **可解释排序**：主题匹配为主，期刊方向、时效和引用速度为辅。
5. **多样性控制**：同一期刊最多 2 篇，同一技术赛道最多 4 篇，延伸相关论文最多 2 篇。

每条推荐都会保存技术赛道、期刊对口度、推荐理由和分项得分，便于审查。

## 邮件阅读体验

- 首屏显示精选数量、对口候选数量和中文摘要数量。
- 目录可跳转到对应论文。
...
```text
recommendation_engine.py  多源检索、去重、准入、评分与选取
