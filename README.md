# AI for Protein 每日文献推送

每天自动从多个学术数据源检索蛋白质领域的新 AI 模型与方法学论文，经主题准入、期刊适配、去重和多样性排序后，通过邮件推送轻阅读日报。酶改造仍被保留，但不再是唯一范围。
...
3. **AI 方法硬门槛**：主通道必须同时包含蛋白质研究对象、AI 方法和明确任务；数据库介绍、纯医学治疗、材料催化等噪声会被排除。
4. **方法学优先排序**：新模型、框架、预训练、表征学习、benchmark 和泛化能力获得独立方法学得分。
5. **任务覆盖**：覆盖蛋白质基础模型、生成式设计、蛋白–蛋白相互作用、蛋白–小分子结合、结构/功能预测、活性/稳定性/选择性预测和 AI 辅助酶工程。
6. **原创研究优先**：综述、观点和文献计量文章会降权，每天最多 2 篇。
7. **多样性控制**：同一期刊最多 2 篇，同一技术赛道最多 4 篇；传统非 AI 酶工程论文每天最多 2 篇。

每条推荐都会保存技术赛道、AI 方法学标记、期刊对口度、推荐理由和“主题/方法/期刊/时效”分项得分，便于审查。

## 邮件阅读体验

- 首屏明确标注 AI for Protein，并显示精选数量、对口候选数量和中文摘要数量。
- 目录可跳转到对应论文。
